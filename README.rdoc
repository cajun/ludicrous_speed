...
=== Ok dude you didn't do anything new.  Wait named_yield_or major difference.  It uses blocks
  
  Foo.w00t { Foo.first } # => returns the first record where shoes = 'carpet'
  Foo.omg.first # => see comment above
...
=== Now what can you do with it?
  ...
  Foo.active.with_bacon.with_limit( 100 ).has_bar( bar )
